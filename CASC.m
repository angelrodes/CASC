...
min_t=min(ages-3*uncertainties-minimum_time_between_units*max_units);
max_t=max(ages+3*uncertainties+minimum_time_between_units*max_units);
% Time resolution
minimum_n_data=1000;
maximum_n_data=1e6;
...
for seq=unique_units'
    ...
    % plot bottom line
    plot(x,seq+y*0,'-','Color',[0.8 0.8 0.8],'LineWidth',2)
end
...
for seq=unique(unit_order)'
    name_seq=unit_names(find(seq==unit_order,1,'first'));
    text(max_tplot,seq+offset,name_seq)
end
% text(max_tplot,min(unique_units)+offset,'Start')
...
for seq=unique_units'
    ...
    % plot bottom line
    plot(x,seq+x*0,'-','Color',[0.8 0.8 0.8],'LineWidth',1)
end
